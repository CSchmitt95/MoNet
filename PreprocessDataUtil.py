...
def getWindowsFromLine32(line, movements):
    ...
    shape_x = len(movements) +  PreprocessingConstants.QUATERNIONS_PER_WINDOW
    shape_y = int((len(pure_data) - PreprocessingConstants.QUATERNIONS_PER_WINDOW)/4)
    ...
    for row in range(0, shape_y):
        ...
        for k in range(0, PreprocessingConstants.QUATERNIONS_PER_WINDOW):
            windows[row, len(movements) + k] = pure_data[row*4 + k]
    return windows
    
def getNumberOfWindowsFromLine32(line):
    pure_data = line[0:3]
    return int((len(pure_data) - PreprocessingConstants.QUATERNIONS_PER_WINDOW)/4)

def generateWindowHeader(sensor_number):
    header = []
    if sensor_number == 0:
        header.append("MovementName")
    for i in range(0, PreprocessingConstants.SAMPLES_PER_SECOND*PreprocessingConstants.WINDOW_SIZE_IN_S):
...
def normalizeWindows32(windows):
    movement_count = len(windows[0]) % PreprocessingConstants.QUATERNIONS_PER_WINDOW
    with Bar("normalisiere Windows", max=len(windows)) as bar:
        for row in windows:
...
def generateWindowHeader32(movements, number_of_sensors):
    ...
    for i in range (0, number_of_sensors):
        for j in range (0, PreprocessingConstants.SAMPLES_PER_SECOND*PreprocessingConstants.WINDOW_SIZE_IN_S):
...
def getCombinedWindowsOf32(windowed_data):
    ...
    number_of_movements = len(sensor_data[0]) % PreprocessingConstants.QUATERNIONS_PER_WINDOW
    width = number_of_sensors*PreprocessingConstants.QUATERNIONS_PER_WINDOW + number_of_movements
